...
function LogManager() {
  window.id = 0;
  window.maxDate = (new Date(Date.UTC(9999, 11, 31, 23, 59, 59, 999))).getTime();
}

LogManager.prototype.connect = function (appToken) {
  search(appToken);
};

function search(appToken) {
  var host = 'http://43532227db0942388c5a047e7bdb0db5.cloudapp.net';
  var pathAndQuery = '/api/beta/search?token=' + appToken;
  ...
  request.onreadystatechange = function () {
    if (request.readyState == 4) {
      ...
      var parsedResponseArray = CryptoJS.enc.Base64.parse(request.responseText);
      var splittedResponse = parsedResponseArray.toString(CryptoJS.enc.Utf8).split('::');
      connect(splittedResponse[0], splittedResponse[1], 'tabletoinsertentity');
    }
  };
  request.open(method, url, true);
  request.setRequestHeader('x-auth', createAuthHeaderForAppToken(appToken, dateHeaderValue, pathAndQuery));
  request.setRequestHeader('x-datetime', dateHeaderValue);
  request.setRequestHeader('x-app', appToken);
  request.send();
}
...
function connect(account, key, table) {
  ...
  request.setRequestHeader('Authorization', createAuthHeader(account, key, table, url, method, contentType, dateHeaderValue));
  request.setRequestHeader('Content-type', contentType);
  request.setRequestHeader('DataServiceVersion', '3.0;NetFx');
  request.setRequestHeader('MaxDataServiceVersion', '3.0;NetFx');
  request.setRequestHeader('x-ms-date', dateHeaderValue);
  request.setRequestHeader('x-ms-version', '2013-08-15');
  request.send(getBody());
}

function createAuthHeader(account, key, table, url, method, contentType, dateHeader) {
  var canResource = getCannonicalResource(account, table);
...
function getCannonicalResource(account, table) {
...
function getBody() {
  window.id += 1;
  var now = new Date();
  var rowKey = ((window.maxDate - now.getTime()) * 10000) + '-' + window.id;
  var partitionKey = rowKey.substring(0, 5);
  var eventTime = (now.getMonth() + 1) + '/' + now.getDate() + '/' + now.getFullYear() + ' '
                  + now.getHours() + ':' + now.getMinutes() + ':' + now.getSeconds();

  return JSON.stringify({ Message: 'test', EventTime: now, RowKey: rowKey, PartitionKey: partitionKey });
}

//connect('pourtest', '4iYhBQC8le0aXjUBDTXd61JaL+532uTwCGrrhXMtk4cr4+B0naa9B8VPSjQpvat9UQnaSlFILABuWtrdgAHvpw==', 'tabletoinsertentity');
(new LogManager()).connect('28a96c5f85ee2e2c912f9d5fc97818075loFm05qhhyDXj9OfzZBpBllEFzNU4Dx');